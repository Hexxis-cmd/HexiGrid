...
6. Report the exact verified result and any platform limitation. Keep instructions short, direct, and tied to visible controls.`;
  ...
  window.HEXIGRID_GUIDES = {
    ...
    live: topic('Live camera, voice, and screen', 'Talk with one agent at a time using browser media when your device supports it.', '5 minutes', 'speech', 'This view is a local live session. Your camera and microphone do not start until you press Start camera + mic, and text always remains available.', [
      ...
      ['Start a realtime provider call', 'If a connected service supports an OpenAI-compatible Realtime API, mark Realtime voice when connecting it. Choose its realtime model in Live, press Start realtime call, approve provider use, and allow the microphone. Share live visual context is separate and off by default; it sends small snapshots from the visible preview at the rate you choose.'],
      ['Create a consented custom voice', 'Open Create a custom voice, choose the provider, add the provider-required spoken consent recording and a clear sample, then confirm the speaker agreed. Eligible providers return one real custom voice ID; review it with the preview buttons before saving.'],
      ['Add an animated or 3D avatar', 'Edit the agent for a small animated GIF or WebP picture. For a 3D avatar, choose one self-contained GLB file under 8 MB; it appears in Live and loads only when needed.'],
      ['Use the agent’s working email', 'Edit the agent and enter their real iLands email address. In Live, Message the actual agent can send mail, check or watch for replies while the page is open, and send a temporary call invitation. Google Mail permission is separate and HexiGrid never asks for the agent’s password.'],
      ...
      ['Call from another network', 'On the host computer, install cloudflared, create an owner passcode, switch to Connect mode, then open Settings and choose Open temporary remote link. Share its address and rotating pairing code privately. Close the link when the call ends.'],
    ...
    ], [['The camera or mic will not start', 'Check the browser permission icon beside the address bar. If the browser or device does not support the API, keep using text and voice output.'], ['Agent email is unavailable', 'Add the agent’s working address to their profile, allow popups, then approve the separate Google Mail permission. Do not enter the agent’s password.'], ['Realtime call is unavailable', 'Edit or reconnect the API service and turn on Realtime voice, then choose a model whose name contains realtime. The provider must support the OpenAI-compatible WebRTC endpoint and may require a paid or eligible account.'], ['Custom voice creation was rejected', 'Custom voices are limited to eligible provider accounts. Confirm both files are supported audio under 10 MB and use the exact consent phrase required by that provider.'], ['The 3D avatar will not load', 'Use a self-contained GLB 2.0 file under 8 MB. Models that point to outside textures or files are rejected on purpose.'], ['Speech recognition is missing', 'That is normal on some browsers, especially Safari and iOS. Use the message box; no Node.js or desktop computer is needed.'], ['The remote link will not open', 'Install cloudflared from Cloudflare, create an owner passcode, use Connect mode, and try again. Quick Tunnels are temporary and have no uptime promise.']], 'HexiGrid never starts a recording or uploads camera and microphone data automatically.', { officialLinks: [['iLands platform capabilities', 'https://ilands.ai/platform'], ['iLands BYOA Runner', 'https://ilands.ai/byoa'], ['Gmail API', 'https://developers.google.com/gmail/api/guides'], ['Cloudflare Quick Tunnels', 'https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/'], ['Install cloudflared', 'https://developers.cloudflare.com/tunnel/downloads/']] }),
